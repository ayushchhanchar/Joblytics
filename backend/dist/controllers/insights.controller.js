...
exports.getMonthlyGoalProgress = exports.getResponseTimeAnalytics = exports.getApplicationStats = exports.getInsights = void 0;
...
const getInsights = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const allApplications = yield prisma_1.Client.application.findMany({
            where: {
                userId: userId,
            },
            orderBy: {
        ...
        const previousApplications = yield prisma_1.Client.application.findMany({
            where: {
                userId: userId,
                appliedAt: {
        ...
        const weeklyTrend = calculateWeeklyTrend(applications, parseInt(timeRange));
...
const getResponseTimeAnalytics = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        const userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.userId;
        if (!userId) {
...
exports.getResponseTimeAnalytics = getResponseTimeAnalytics;
const getMonthlyGoalProgress = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        const userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.userId;
        if (!userId) {
        ...
        const monthlyApplications = yield prisma_1.Client.application.findMany({
            where: {
                userId: userId,
                appliedAt: {
                    gte: monthStart,
...
exports.getMonthlyGoalProgress = getMonthlyGoalProgress;
// Helper functions
function calculateMetrics(current, previous) {
    const totalApplications = current.length;
    const previousTotal = previous.length;
    const responseCount = current.filter(app => ['INTERVIEWING', 'OFFER'].includes(app.status)).length;
    ...
    const applicationsTrend = previousTotal > 0
        ...
        : 0;
    const previousResponseCount = previous.filter(app => ['INTERVIEWING', 'OFFER'].includes(app.status)).length;
...
function calculateWeeklyTrend(applications, timeRange) {
    const weeks = [];
    const now = new Date();
...
function calculateTopCompanies(applications) {
...
function calculateTopLocations(applications) {
...
function generatePerformanceInsights(current, all) {
    const insights = [];
...
function calculateGoalProgress(applications) {
    const now = new Date();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
